...
export default function withPages(config) {
  ...
  const tree = dirTree(srcPagesDir, {
    ...
    const outFilePath = resolve(outDir + '/pages/' + relativePath, '../')
...
/*
  ...
  */
  ...
  config.publicDir = projectRoot+'/public'
  return config
}
 ...
 * @param {string} path 
 *
function addPageTree(treeObj, path){
  const dirList = path.split('/'), deep = dirList.length - 1
  let currentNode = treeObj
  for(let i=0;i<deep;i++){
    currentNode[dirList[i]] ??= {}
    currentNode = currentNode[dirList[i]]
  }
  currentNode.name = [dirList[deep]].replace(/.[\w]+$/, '')
  currentNode.
}
*/
